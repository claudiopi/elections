...
export class CandidatesAndVotesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.stateService.currentlyChosenYear.pipe(takeUntil(this.destroy$)).subscribe(
      () => { this.uSElectionsDataService.getVoteResultList(this.stateService.currentlyChosenYear.value as any).pipe(take(1)).subscribe(
        data => this.uSElectionsDataVoteResult = data
    )});
    this.stateService.currentlyChosenYear.pipe(takeUntil(this.destroy$)).subscribe(
      () => { this.uSElectionsDataService.getVoteResultList1(this.stateService.currentlyChosenYear.value as any).pipe(take(1)).subscribe(
        data => this.uSElectionsDataVoteResult1 = data
    )});
    this.stateService.currentlyChosenYear.pipe(takeUntil(this.destroy$)).subscribe(
      () => { this.uSElectionsDataService.getCandidateVoteResultList(this.stateService.currentlyChosenYear.value as any).pipe(take(1)).subscribe(
        data => this.uSElectionsDataCandidateVoteResult = data
    )});
    this.stateService.currentlyChosenYear.pipe(takeUntil(this.destroy$)).subscribe(
      () => { this.uSElectionsDataService.getStateVoteResultList(this.stateService.currentlyChosenYear.value as any).pipe(take(1)).subscribe(
        data => this.uSElectionsDataStateVoteResult = data
    )});
  }
